...
class FirebaseAuthManager {
    ...
    async checkEmailAndDeviceVerification() {
        if (!this.currentUser) return false;
        
        // Reload user to get fresh verification status
        await this.currentUser.reload();
        
        // Check if email is verified AND device is verified
        const emailVerified = this.currentUser.emailVerified;
        const deviceVerified = this.isCurrentDeviceVerified();
        
        console.log('Verification status:', { emailVerified, deviceVerified });
        
        // Both email and device must be verified
        if (emailVerified && !deviceVerified) {
            // Email is verified, mark device as verified too
