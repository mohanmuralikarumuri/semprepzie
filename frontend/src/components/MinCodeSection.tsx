...
const MinCodeSection: React.FC<MinCodeSectionProps> = ({ darkMode = false, onEditorStateChange, source = 'standalone' }) => {
  ...
  const [selectedProgram, setSelectedProgram] = useState<MinCodeProgram | null>(null);
  ...
  // Load min code subjects from database on mount
  useEffect(() => {
  ...
  const executeCode = async (codeToExecute?: string, inputToUse?: string) => {
    ...
    try {
      const result = await codeExecutionService.executeCode({
        language: selectedProgram.language as 'c' | 'cpp' | 'python' | 'java',
        code: code,
        stdin: stdin
      });
  ...
  const selectSubject = (subject: MinCodeSubject) => {
    setSelectedSubject(subject);
    setView('programs');
  };

  const selectProgram = (program: MinCodeProgram, options?: { autoRun?: boolean }) => {
    setSelectedProgram(program);
    setCurrentCode(program.code);
    setInput(program.sample_input || '');
    setOutput('');
    setView('editor');
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2 sm:space-x-4 min-w-0 flex-1">
                ...
                <div className="flex items-center space-x-2 min-w-0">
                  <Code className={`w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 ${darkMode ? 'text-purple-400' : 'text-purple-600'}`} />
                  <h1 className="text-lg sm:text-2xl font-bold truncate">
                    {view === 'subjects' && 'Minimum Code Examples'}
            ...
            {loadingSubjects ? (
              ...
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  ...
                  <p className={`text-lg ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>No min code subjects available</p>
                  <p className={`text-sm mt-2 ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Contact admin to add subjects</p>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {minCodeSubjects.map((subject) => (
                  <div
                    key={subject.id}
          ...
          <div className="p-3 sm:p-6">
            {loading ? (
                ...
                {programs.map((program) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3 gap-2">
                      ...
                      <span className="text-xs bg-purple-600 px-2 py-1 rounded uppercase font-mono text-white flex-shrink-0">
                        {program.language}
                      </span>
                    </div>
                    {program.description && (
              ...
              <div className="text-center py-12">
                ...
                <p className={darkMode ? 'text-gray-400' : 'text-gray-500'}>No min code examples available for this subject yet.</p>
              </div>
            )}
          </div>
        )}
