...
const LabSection: React.FC<LabSectionProps> = ({ darkMode = false, onEditorStateChange }) => {
  ...
  const labSubjects: LabSubject[] = [
    ...
    },
    {
      ...
      description: 'Search algorithms, AI techniques, and problem solving',
      icon: '🤖'
    },
    {
      id: 'fsd',
      name: 'Full Stack Development-2',
      description: 'Web development, APIs, and full stack applications',
      icon: '💻'
    }
  ];
  ...
  useEffect(() => {
    if (selectedSubject) {
      loadPrograms(selectedSubject.id);
    }
  }, [selectedSubject]);
  ...
  const loadPrograms = async (subjectId: string) => {
    try {
      setLoading(true);
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:3001';
      const response = await fetch(`${apiUrl}/api/lab-programs/subject/${subjectId}`);
